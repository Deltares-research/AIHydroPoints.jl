...
function pim_make_ts(values::Matrix{Float32}, quantity::String; nstations=size(values,1))
...
function pim_make_data(; nstations=3, ntimes=60)
    ...
    input = Dict{String, TimeSeries}(
        "tide"  => pim_make_ts(tide_vals,  "tide";  nstations),
        "surge" => pim_make_ts(surge_vals, "surge"; nstations),
    )
    target = Dict{String, TimeSeries}(
        "waterlevel" => pim_make_ts(wl_vals, "waterlevel"; nstations),
    )
    return input, target
end

function make_pim_settings(; nstations=3)
...
@testset "ProductInteractionModel construction" begin
    m = ProductInteractionModel(make_pim_settings())
    ...
    @test get_flux_model(m) isa ProductInteractionFlux
    ...
    @test_throws AssertionError ProductInteractionModel(bad)
end
...
@testset "ProductInteractionModel preprocess" begin
    nstations = 3; ntimes = 50; nlags = 8
    m = ProductInteractionModel(make_pim_settings(nstations=nstations))
    input, _ = pim_make_data(nstations=nstations, ntimes=ntimes)
    ...
    # Each column of x_station is a one-hot vector
...
@testset "ProductInteractionModel forward" begin
...
@testset "ProductInteractionModel train_model!" begin
    nstations = 3; ntimes = 80
    m = ProductInteractionModel(make_pim_settings(nstations=nstations))
    input, target = pim_make_data(nstations=nstations, ntimes=ntimes)
    ...
    m2 = ProductInteractionModel(make_pim_settings(nstations=nstations))
...
@testset "ProductInteractionModel predict" begin
...
@testset "ProductInteractionModel save/load params" begin
    m  = ProductInteractionModel(make_pim_settings())
    fn = joinpath(temp_dir, "product_interaction_params.jld2")
    ...
    m2 = ProductInteractionModel(make_pim_settings())
    load_params!(m2, fn)
