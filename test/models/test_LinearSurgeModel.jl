...
"""Forcing-only input dict for LinearSurgeModel."""
function make_surge_input(; nwind=3, ntimes=30)
    wind_x   = make_ts(randn(Float32, nwind, ntimes), "wind_x")
    wind_y   = make_ts(randn(Float32, nwind, ntimes), "wind_y")
...
@testset "LinearSurgeModel preprocess" begin
    ...
    # Tensor shape: (1, 3*nwind, nlags, nvalid)
    @test size(tensor) == (1, 3*nwind, nlags, nvalid)
    @test eltype(tensor) == Float32

    # Output dict has the right structure
    @test haskey(output, "surge")
    ts = output["surge"]
    @test ts isa TimeSeries
    @test size(ts.values) == (nstations, nvalid)
    @test all(ts.values .== 0f0)                                     # zero-initialised
    @test length(get_times(ts)) == nvalid
    @test get_names(ts) == m.settings["out_names"]                   # metadata from settings
end
