...
function inter_make_ts(values::Matrix{Float32}, quantity::String; nstations=size(values,1))
...
function inter_make_data(; nstations=3, ntimes=60)
    ...
    input = Dict{String, TimeSeries}(
        "tide"  => inter_make_ts(tide_vals,  "tide";  nstations),
        "surge" => inter_make_ts(surge_vals, "surge"; nstations),
    )
    target = Dict{String, TimeSeries}(
        "waterlevel" => inter_make_ts(wl_vals, "waterlevel"; nstations),
    )
    return input, target
end
