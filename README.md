...
The AI model in this folder comtains 3 modules for forecasting, tides, surges and their interaction. 
...
The inputs for winds and air-pressure are sampled at a few relavant locations. In the examples ERA5 fields from the Copernicus Climate Data Store (CDS) are used. Tides require Doodson phases as input, but these are easily computed from the times. The outputs in the examples are from the DCSM-FM model.
...
Currently the three modules tide, surge and interaction are working. There are a few succesful models. However, the code is still messy.
