...
# Deep-O_net architecture with modulation based on station parameters 
# The branch_net processes the input data (wind time series) and provides output features on
# a given flattened shape.
# The trunk_net is simplified to just provide station-specific parameters for modulation,
# per station.
struct DeepONet_on_stations{T1,T2}
    station_params::T1 #Simplified trunk_net, since we only have fixed locations for output
    branch_net::T2
end

#constructor
DeepONet_on_stations(n_output_stations,nchannel,n_input_vars,n_input_stations,f_activation) = DeepONet_on_stations(
    # station_params
    Dense(n_output_stations=>(nchannel[end]),relu;bias=false),
    # branch_net
    Chain(
        Conv((2,),(n_input_stations*n_input_vars)=>nchannel[1],f_activation,stride=(2,)),
        Conv((2,),nchannel[1]=>nchannel[2],f_activation,stride=(2,)),
        Conv((2,),nchannel[2]=>nchannel[3],f_activation,stride=(2,)),
        Conv((2,),nchannel[3]=>nchannel[4],identity,stride=(2,)),
        Flux.flatten
...
# using: 
#   nchannel=(64, 64, 64, 32)
#   n_output_stations=11
#   n_input_vars=2
#   n_input_stations=10
# example input
# stations: (11, 274524) # 11 output stations and 274524 records in the training set
# wind input: (16, 20, 274524) 16 times and 10 wind-stations with 2 vars each
#
# Dense(n_output_stations=>(nchannel[end]),relu;bias=false)
# 10=>32
# branch_net
#
# Chain(
#       (16, 20, 274524) -> (8,64,274524)
#     Conv((2,),n_input_channels=>nchannel[1],f_activation,stride=(2,)),
#       (8,64,274524) -> (4,64,274524)
#     Conv((2,),nchannel[1]=>nchannel[2],f_activation,stride=(2,)),
#       (4,64,274524) -> (2,64,274524)
#     Conv((2,),nchannel[2]=>nchannel[3],f_activation,stride=(2,)),
#       (2,64,274524) -> (1,32,274524)
#     Conv((2,),nchannel[3]=>nchannel[4],identity,stride=(2,)),
#       (1,32,274524) -> (32,274524)
#     Flux.flatten
...
function (l::DeepONet_on_stations)(x)
    x_station, x_input = x 
    # Step 1: branch net to get station independent features
    x1= l.branch_net(x_input)
    # Step 2: Get station parameters for modulation
    s1 = l.station_params(x_station) 
    s1 = reshape(s1, size(x1)) # reshape the output of station_params to match x1
    # Step 3: Modulate the output of Conv layer with station parameters
    z1 = sum(s1 .* x1, dims=1) # inner product over feature dimension
    return z1
end


model_cpu = DeepONet_on_stations(n_output_stations,nchannel,n_input_vars,n_input_stations,f_activation)
...
# y_pred=model((training_x_station_cpu,training_x_input_cpu))
# test_loss=compute_loss(model_cpu,training_x_station_cpu,training_x_input_cpu,training_y_cpu)
